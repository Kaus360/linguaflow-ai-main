...
class PunjabiPipeline(LanguagePipeline):
    def __init__(self):
        self.examples = []
        try:
            dataset_path = os.path.join(os.path.dirname(__file__), '..', 'datasets', 'punjabi_examples.json')
    ...
    def execute(self, text: str) -> dict:
        if not text.strip():
            return {"language": "pa-IN", "stage2_text": ""}
        
        safe_text = text.replace('\n', ' ')[:1000]
        local_text = correct_text_stage2_local(safe_text, "pa-IN")
        api_key = os.getenv('HF_API_KEY', '').strip()
        if not api_key or api_key == 'hf_dummy_key' or api_key.startswith('your_'):
            return {"language": "pa-IN", "stage2_text": local_text}
        ...
        prompt = f"Correct the grammar and semantics of the following Punjabi sentence.\n{examples_str}Incorrect: '{safe_text}'\nCorrect:"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(output, list) and 'generated_text' in output[0]:
                    ...
                    return {"language": "pa-IN", "stage2_text": corrected_text}
